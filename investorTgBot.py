...
def bot_session():
    ...
    while i < 3600 and response_timestamp < session_timestamp:
        ...
        if i == 2700:
            requests.get(f"{TELEGRAM_API_URL}{TOKEN}/sendMessage", data={"chat_id": MY_CHAT_ID, "text": "Turning off in 15 minutes⏳"})

        i += 1

    if response_timestamp < session_timestamp and chat_id == MY_CHAT_ID:
        requests.get(f"{TELEGRAM_API_URL}{TOKEN}/sendMessage", data={"chat_id": MY_CHAT_ID, "text": "Okay see you next month I guess!"})
    else:
        salary = user_input.get("result", [{}])[0].get("message", {}).get("text", "")

        result = investLogic(float(salary))
        requests.get(f"{TELEGRAM_API_URL}{TOKEN}/sendMessage", data={"chat_id": MY_CHAT_ID, "text": f"Calculations for {salary} ILS:\n{result}"})
